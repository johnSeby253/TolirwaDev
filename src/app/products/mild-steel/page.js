...
import { MILD_STEEL_SHEETS_CONTENT } from "@/lib/ApiPath";
...
const Steel = () => {
  ...
  const [mdSheetData,setMDSheetData]=useState({});
  const [loading,setLoading]=useState(true);
    ...
    const fetchMDsheetData=async()=>{
      try{
        const res = await client.query({
          query: MILD_STEEL_SHEETS_CONTENT,
        });
        const pageData = res.data.pageBy;
        const result = formatSheetsPage(pageData);
        console.log("RES",result);
        
        setMDSheetData(result);
      }catch(err){
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-2/3 text-[#56565A] font-source_Sans">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {mdSheetData.items?.map((steel) => (
                <Link
                  key={steel.id}
                  href={`/products/mild-steel/${steel.id}`}
                ...
                >
                  <div className="flex items-center justify-center bg-[#CCCCCC] group-hover:bg-[#ef3713] h-12">
                    <h1 className="font-bold">{steel.title}</h1>
                  </div>
